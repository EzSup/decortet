...
export default function CartPage() {
	...
	function calculateSum(items: ProductWithCount[]): number {
		return items.reduce(
			(total, item) => total + (item.product.price ?? 1) * item.quantity,
			0
		);
	}
	...
	const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
		...
		axios.post(`/api/User/Buy`, formData).then((response: AxiosResponse) => {
			...
			<div id="page">
				...
				>
					...
					<form onSubmit={handleSubmit}>
						...
						<p>
							<i>
								Увага! Оплата замовлення здійснюється{" "}
								<strong>лише готівкою при отриманні!</strong>
							</i>
						</p>
